...
async function getProduct(siteId: string, slug: string, locale: string, year: number | null) {
  ...
  // Get tags from template or product
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const canonicalUrl = `${baseUrl}/${locale}/shop/planners/${year}/${slug}`;
...
export default async function PlannerProductPage({ params }: PageProps) {
  const { locale, year, slug } = await params;
  ...
  // Validate that the product type matches this route (planner)
  // Only enforce if productType is explicitly set to something other than 'planner'
  if (product.productType && product.productType !== 'planner') {
    notFound();
  }

  // Check if customer owns this product
        ...
        <main className="flex-1">
          <ProductDetailPage
            ...
            breadcrumbItems={[
              ...
              { label: yearLabel, href: `/${locale}/shop/planners/${year}` },
              { label: product.name },
            ]}
          />
        </main>
