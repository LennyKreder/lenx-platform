...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const canonicalUrl = `${baseUrl}/${locale}/shop/templates/${slug}`;
...
export default async function TemplateProductPage({ params }: PageProps) {
  ...
  // Validate that the product type matches this route (template)
  if (product.productType && product.productType !== 'template') {
    notFound();
  }

  // If the URL slug doesn't match the locale's primary slug, redirect
  if (product.slug && product.slug !== slug) {
    redirect(buildUndatedProductUrl(locale, 'templates', product.slug));
  }
        ...
        <main className="flex-1">
          <ProductDetailPage
            ...
            breadcrumbItems={[
              ...
              { label: 'Templates', href: `/${locale}/shop/templates` },
              { label: product.name },
            ]}
          />
        </main>
