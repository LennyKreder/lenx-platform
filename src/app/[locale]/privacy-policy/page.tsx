...
async function getPageContent(siteId: string, locale: string): Promise<string | null> {
  const page = await prisma.contentPage.findFirst({
    where: { siteId, slug: 'privacy-policy' },
    include: {
      translations: true,
    },
  });
...
export async function generateMetadata({ params }: PageProps) {
  ...
  const title = translation?.title || 'Privacy Policy';
  const ogTitle = `${title} - ${site.name}`;
  const description = translation?.metaDescription || `Privacy policy for ${site.name}`;

  return {
    title,
    description,
    alternates: await buildAlternates(locale, '/privacy-policy', { baseUrl, siteId: site.id }),
    openGraph: {
      title: ogTitle,
      description,
      type: 'website',
      url: `${baseUrl}/${locale}${translatePath('/privacy-policy', locale as Locale)}`,
      siteName: site.name,
    },
    twitter: {
