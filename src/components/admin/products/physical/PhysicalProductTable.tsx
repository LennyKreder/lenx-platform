...
import { Pencil, Trash2, CopyPlus, Image as ImageIcon, Video, Star } from 'lucide-react';
...
interface PhysicalProductTableProps {
...
export function PhysicalProductTable({
  products,
  pagination,
  onPageChange,
  selectedIds,
  ...
  const columns: Column<PhysicalProduct>[] = [
    ...
    },
    {
      ...
      header: 'Product',
      cell: (product) => {
        const name = getTranslation(product.translations, 'en') || 'Unnamed Product';
        return (
          <div>
            <Link
    ...
    },
    {
      ...
      cell: (product) => (
        <div>
          <span>{product.currency} {(product.priceInCents / 100).toFixed(2)}</span>
          {product.compareAtPriceInCents != null && product.compareAtPriceInCents > 0 && (
            <div className="text-xs text-muted-foreground line-through">
              {product.currency} {(product.compareAtPriceInCents / 100).toFixed(2)}
            </div>
          )}
        </div>
      ),
