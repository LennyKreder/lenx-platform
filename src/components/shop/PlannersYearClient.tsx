...
import { type YearSegment, type DatedProductType, buildDatedProductUrl } from '@/lib/routing';
...
interface PlannersYearClientProps {
  locale: string;
  year: YearSegment;
  productType: DatedProductType;
}
...
export function PlannersYearClient({
  locale,
  year,
  productType,
}: PlannersYearClientProps) {
  ...
  useEffect(() => {
    async function fetchProducts() {
      ...
      try {
        const params = new URLSearchParams({
          ...
          undated: year === 'undated' ? 'true' : 'false',
        });
    ...
    fetchProducts();
  }, [locale, year, productType, debouncedSearch, selectedTheme, selectedThemeMode, selectedDevice]);
  ...
  const gridProducts = products.map((p) => ({
    ...p,
    // Build the URL for the new routing structure
    href: buildDatedProductUrl(locale, productType, year, p.slug),
